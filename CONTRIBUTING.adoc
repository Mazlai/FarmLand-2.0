...
Ces tests sont automatiquement exécutés lors de la création d'une Pull Request vers les branches 'develop' et 'main' afin d'attester que seul du code stable est poussé sur ces dernières.
...
// * Workflow release :
//   * Lint code :
//     * Linter le code avant de push une branche pour éviter problèmes de CI/CD (+ mise en forme auto formatage code)
//     * Lint code dans pipeline (bloque si erreurs)
//   * Tests e2e exécutés en même temps que les tests unitaires existants
//   * Déploiement prod / front sur GitHub page (artifact créé)
// * Mise en place logs (back)
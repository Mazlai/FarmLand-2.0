...
export const signUpAsync = async (
  ...
  try {
    if (await User.findOne({ where: { email: req.body.email } }))
      res
        .status(403)
        .json({ message: "User with the same mail address already exists" });
    else {
...
export const verifyTokenAsync = async (
  ...
  res
    .status(200)
    .json({ message: "Valid JWT token", userId: tokenValidation.user?.id });
};
...
/** Update current user. */
export const updateCurrentUserAsync = async (
  req: Request,
  res: Response,
): Promise<void> => {
  try {
    const user = await getUserFromReqTokenAsync(req, res);
    if (user) {
      const { firstName, lastName, birthDate, email, phone, gender, password } =
        req.body;
...
/** Delete current user. */
export const deleteCurrentUserAsync = async (
  req: Request,
  res: Response,
): Promise<void> => {
  try {
 ...
 */
async function verifyUserTokenAsync(
  token: string,
): Promise<{ status: number; user?: User; error?: string }> {
  try {
    // Missing token
    if (!token || !/\S/.test(token))
      return { status: 401, error: "Missing JWT token" };
    ...
    if (error instanceof jwt.JsonWebTokenError)
      return { status: 401, error: "Invalid JWT token" };
    // Expired token
    else if (error instanceof jwt.TokenExpiredError)
      return { status: 401, error: "Expired JWT token" };
 ...
 */
async function getUserFromReqTokenAsync(
  req: Request,
  res: Response,
): Promise<User | undefined> {
  const tokenValidation = await verifyUserTokenAsync(

...
export const validateUserCreation = (
  ...
  const { firstName, _lastName, birthDate, email, gender, password } = req.body;
  const errors: string[] = [];
  ...
  if (!birthDate || isNaN(Date.parse(birthDate))) {
    ...
    if (isNaN(birthDateObj.getTime()) || age < 0 || age > 120) {
      errors.push(
        "La date de naissance doit être valide et l'âge doit être compris entre 0 et 120 ans.",
      );
    }
  }
  ...
  if (!gender || !validGenders.includes(gender.toLowerCase())) {
    errors.push(
      "Le genre doit être: male, female, other, homme, femme ou autre",
    );
  }
